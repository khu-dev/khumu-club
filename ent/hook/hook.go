...
// The LikeClubFunc type is an adapter to allow the use of ordinary
// function as LikeClub mutator.
type LikeClubFunc func(context.Context, *ent.LikeClubMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f LikeClubFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	mv, ok := m.(*ent.LikeClubMutation)
	if !ok {
		return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.LikeClubMutation", m)
	}
	return f(ctx, mv)
}
